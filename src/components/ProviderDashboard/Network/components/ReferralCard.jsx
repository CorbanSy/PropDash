//levlpro-mvp\src\components\ProviderDashboard\Network\components\ReferralCard.jsx
...
export default function ReferralCard({ referral, onRefresh }) {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return {
          badge: "bg-slate-100 text-secondary-700 border-slate-200",
          icon: <Send size={14} />,
          text: status,
        };
    }
    ...
    <div className={`${theme.card.base} ${theme.card.padding} relative`}>
      <div className="flex items-start justify-between gap-4 mb-4">
        ...
        <div className="flex items-start gap-3 flex-1">
          <div className="bg-gradient-to-br from-primary-100 to-purple-100 text-primary-700 w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="relative">
            ...
            >
              <MoreVertical size={18} className="text-secondary-600" />
            </button>
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-slate-200 py-2 z-20">
                  {referral.status === "pending" && (
                    <button
                      ...
                      className={`w-full px-4 py-2 text-left ${theme.text.caption} text-secondary-700 hover:bg-slate-100 flex items-center gap-2`}
                    >
                      <Send size={16} />
                      Resend Invitation
                    </button>
                  ...
                  <button
                    ...
                    className={`w-full px-4 py-2 text-left ${theme.text.caption} text-error-600 hover:bg-error-50 flex items-center gap-2`}
                  >
                    <XCircle size={16} />
                    Remove
                  </button>
      ...
      {referral.status === "joined" && (
        <div className="mt-4 pt-4 border-t border-slate-200">
          <p className={`${theme.text.caption} font-semibold text-secondary-700 mb-3`}>Referral Progress</p>
                ...
                {index < lifecycle.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-1 ${
                      step.complete ? "bg-success-300" : "bg-slate-200"
                    }`}
                  />
                )}
              </div>
          ...
          <div className={`flex items-center justify-between mt-2 ${theme.text.caption}`}>
            <span>Invited</span>
            <span>Paid</span>
          </div>
        </div>
      ...
      {referral.status === "joined" && (
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className="bg-primary-50 rounded-lg p-2 text-center">
            <p className="text-xs text-primary-700 mb-1">Jobs</p>
            <p className="text-lg font-bold text-primary-900">{referral.jobsReferred || 0}</p>
          </div>
      ...
      {referral.status === "pending" && (
        <button
          onClick={handleResend}
          className={`w-full mt-4 py-2 ${theme.text.caption} text-primary-600 hover:text-primary-700 font-medium transition flex items-center justify-center gap-2 border-t border-slate-200 pt-4`}
        >
          <Send size={14} />
          Resend Invitation
        </button>
