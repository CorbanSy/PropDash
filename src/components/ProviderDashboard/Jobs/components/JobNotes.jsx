//levlpro-mvp\src\components\ProviderDashboard\Jobs\components\JobNotes.jsx
...
export default function JobNotes({ job, onRefresh }) {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${theme.card.base} ${theme.card.padding}`}>
        <h3 className={`${theme.text.h4} mb-3`}>Add Note</h3>
        <div className="flex gap-2">
          <textarea
            ...
            rows={3}
            placeholder="Add details about this job, issues encountered, client requests, etc..."
          />
          <button
      ...
      <div className="space-y-3">
        {notes.length === 0 ? (
          <div className={`${theme.card.base} ${theme.card.padding} text-center py-8`}>
            ...
            <p className="text-sm text-slate-500 mt-1">Add notes to track important job details</p>
          </div>
        ) : (
          notes.map((note) => (
              ...
              {editingId === note.id ? (
                ...
                <>
                  <div className="flex items-start justify-between mb-2">
                    <p className="text-sm text-slate-700 flex-1">{note.note}</p>
